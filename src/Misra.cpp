...
void InitNetRel(
		vector<vector<int> >& netrel1,
		vector<vector<int> >& netrel2,
		vector<vector<int> >& netrel3,
		vector<vector<string> >& EdList,
		vector<SimVar>& SimDAG)
{
    for(size_t i = 0; i < SimDAG.size(); i++)
    {
        vector<int> temprel(SimDAG.size(), 0);
        ...
        for(size_t j=0; j < SimDAG[i].vpi.size(); j++)
        {
...
{
    for(size_t l=0;l<SimDAG.size();l++)
    {
        int y=0;
        ...
        pratio = (config[1])?pratio*(1.0-z):pratio*z;
    }
}

List WriteLandscape(vector<SimVar>& SimDAG, vector<NODE>& Tree, vector<string>& GeneLabels, double& pthres)
{
  double epsilon=0.0001;
  int NVars= SimDAG.size();
	vector<double> NormStat;
	
	vector<bool>config(SimDAG.size(), false);
  ...
  double max2=0.0;
	for(size_t i=0;i<SimDAG.size()-1;i++)
	{
		config[i]=true;
        for(size_t j=i+1;j<SimDAG.size();j++)
        {
            config[j]=true;
            double pratio=1.0;
            ConfigParam( SimDAG,  config,  pratio);
  ...
  double max3=0.0;
	for(size_t i=0;i<SimDAG.size()-2;i++)
	{
		config[i]=true;
        for(size_t j=i+1;j<SimDAG.size()-1;j++)
        {
            config[j]=true;
            for(size_t k=j+1;k<SimDAG.size();k++)
            {
                config[k]=true;
                bool pre=false;
    ...
    for(size_t i=0;i<prob3.size();i++)
    {
        if( present3[i]==true && prob3[i]>=max3*(pthres-epsilon))
        {
            ...
            //pathFile << i+1+NVars+NVars*(NVars-1)/2 << "[label=\"" << SimDAG[plabel3[i][0]].vname << ","<<SimDAG[plabel3[i][1]].vname << ","<<SimDAG[plabel3[i][2]].vname << "\" ,shape=ellipse, style=filled, color=\"0.13000," <<prob3[i]/max3 <<",0.99\",fontsize=\"36\", font=\"Helvetica\", fontcolor=black];"<<endl; 
            int pa;
            double papr=0;
            for(size_t j=0;j<prob2.size();j++)
            {
    ...
    for(size_t i=0;i<prob2.size();i++)
    {
        ...
        if(prob2[i]>=max2*(pthres-epsilon))
        {
            for(int pr=1;pr<Tree[0].NDes+1;pr++)
            {
                ...
                int mcnt=0;
                for(size_t j=0;j<Tree[pr].gtype.size();j++)
                {
	...
	for(int ng=0;ng<NVars;ng++)
    {
        for(size_t i=0;i<DAG.size();i++)
        {
            if(VarName[i]==SimDAG[ng].vname)
            {
                config[i]=true;
                double pratio=1.0;
                for(size_t l=0;l<DAG.size();l++)
                {
                    int y=0;
                    int y2=VarPi[l].size();
                    for(size_t pa=0;pa<VarPi[l].size();pa++)
                    {
...
/*
 * Read the data file, store taxon and gene names as a vector and partially
...
void  SimulateDAG( vector<NODE>& Tree, vector<SimVar>& SimDAG, vector< vector<bool> >& sdata)
{
    ...
    while(tsamp<sdata.size())
	{
		...
		for(size_t j=0;j<SimDAG.size();j++)
		{
			double x;
            ...
            if(SimDAG[j].vparam[y1]+SimDAG[j].vparam[y1+(int(pow(2.0,y2)))] <.999){cout<<"Prob norm \n";}
            if(SimDAG[j].vparam[y1]+SimDAG[j].vparam[y1+(int(pow(2.0,y2)))] >1.001){cout<<"Prob norm 2 \n";}
            if(z>x)
            {
                data[j]=0;
            }else
        ...
        while(ctrue==false && sdt<sdata.size())
        {
            ctrue=true;
            int snmut=0;
            for(size_t gch=0;gch<SimDAG.size();gch++)
            {
...
void InitEnt(  vector<FAM>& BN, vector<NODE>& Phylogeny)
{
    for(size_t i=0;i<BN.size();i++)
    {
        double nummut=0.0;
        double nvert=0.0;
        for(size_t k=0;k<Phylogeny.size();k++)
        {
...
//Initialize the subtree structure and potential parents for each gene	in variable "Genes" given "Tree"																										*
...
void GlobalPruning( vector<FAM>& BayesN, vector<NODE>& Tree)
{
    ...
    for(size_t i=0;i<Tree[0].gtype.size();i++)
    {
        TempCO.push_back(0);
        famtemp.pi.push_back(false);
    }
    ...
    vector<int> Order;
    for(size_t i=0;i<BayesN.size();i++)
    {
        Order.push_back(i);
        freq[i]=0.0;
        for(size_t j=0;j<Tree.size();j++)
        {
            ...
            for(size_t k=i+1;k<BayesN.size();k++)
            {
...
void SanityCheckTree(vector<NODE>& Tree)
{
    ...
    {
        cout<<"PROBLEM in INITTREE.cpp. Zero has "<<Tree[0].child.size()<<" children";
        throw 2;
    }
...
void InitTree( vector<NODE>& Tree, vector<FAM>& BN)
{
    ...
    for(int i=Ntax;i<2*Ntax-2;i++)
    {
        ...
        for(size_t k=0;k<Tree[0].gtype.size();k++)									/*	each time a new internal vertex is found			*/
        {
    ...
    if(Orphans.size()!=1){cout<<"Problem initializing tree. "<<Orphans.size()<<'\n';}else
    {
    ...
    {
        for(size_t j=0;j<Tree[i].gtype.size();j++)
        {
...
void EliminateRedundantCharacters(vector< vector<bool> >& Data, vector<vector<int> >& char_label)
{
	...
	for(size_t ch1=0;ch1<Data[0].size();ch1++)
	{
		...
		if(RedChar[ch1]==false)
		{
			vector<int> templ;
			templ.push_back(ch1);
			for(size_t k=ch1+1;k<Data[0].size();k++)
			{
				if(RedChar[k]==false)
				{
					RedChar[k]=true;
...
{
	for(size_t nb=0;nb<conf.size();nb++)
	{
		if(conf[nb]==false)
		{
			conf[nb]=true;
...
//Greedy K2 heuristic for updating the parents of gene "G" in "FamG"
//given "Tree"	cf. Cooper & Herskovits (1992).
			...
			if((DAG[g].potpi[i]==1 && DAG[i].potpi[g]==1)|| op=='n')
            {
                if( DAG[g].order>DAG[i].order && FamG.pi[i]==0 )
                {
                    ...
                    for(size_t j=0;j<Tree.size();j++)
                    {
                        ...
                        //int indxi=0;
                        int par=0;
                        for(size_t k=0;k<Tree[j].gtype.size();k++)
                        {
                            if(FamG.pi[k]==1)
                            {
	...
	while(opt==false)
	{
		...
		for(size_t i=0;i<DAG.size()-1;i++)
		{
			if((DAG[Ord[i]].potpi[Ord[i+1]]==true) && (DAG[Ord[i+1]].potpi[Ord[i]]==true))
            {
                ...
                //if(Ord[DAG[i].order]!=i){cout<<"problem in reordering genes for gene"<<i<<'\t';}
                ...
                if((fg1.score+fg2.score)>(DAG[Ord[i]].score+DAG[Ord[i+1]].score))
                {
                    ...
                    // cout<<" to "<<psc<<" reordered "<<scount<<" times \n";
                }else
                {
                    int tempord=Ord[i];
                    DAG[Ord[i]].order+=1;
...
void swapnodes(vector<NODE>& Tree, vector<FAM>& DAG, int& i, int& gramps, int& spart, bool& change)
{
    ...
    for(size_t j=0;j<Tree[i].gtype.size();j++)
    {
        ...
        if((CSOld!=tempold))
        {
            ...
            {
				cout<<"incorrect label for node "<<i<<" sib "<<sib<<" dad "<<Tree[i].dad<<" gramps "<<gramps<<" spart "<<spart<<endl;
				cout<<" Nmut "<<DAG[j].nmut<<endl;
				cout<<"Dad's children "<<Tree[Tree[i].dad].child[0]<<" and "<<Tree[Tree[i].dad].child[1]<<endl;
				cout<<"gramps children "<<Tree[gramps].child[0]<<" and "<<Tree[gramps].child[1]<<endl;
				cout<<"spart's father "<<Tree[spart].dad<<endl;
				cout<<"sib's father "<<Tree[sib].dad<<endl;
				cout<<j<<" gene, node ="<<Tree[i].gtype[j]<<" sib = "<<Tree[sib].gtype[j]<<" dad = "<<Tree[Tree[i].dad].gtype[j]<<" gramps = "<<Tree[gramps].gtype[j]<<" spart = "<<Tree[spart].gtype[j]<<endl;
				throw 2;
            }
        }
        double theta=0.0;
...
List StructLearn(vector<vector<bool> >& Data, 
                       vector<string>& GeneLabels, 
                       vector<SimVar>& SimDAG, 
                       int& NTrees, double& pthres)
{
    ...
    /*
     * Initiate a subroutine for randomly re-seeding the search procedure.
     * Each tree serves as a starting point for a local search heuristic
     * through tree space.
     */
    double bnscore=0.0;
    ...
    for(int Seed=0;Seed<NTrees;Seed++)
    {
        ...
        /*
         * Initialize the missing data values in the  variable 
         * "Phy", cf. InitTree.cpp and perform local pruning
         */
        InitTree(Phy,DAG);
        ...
        /*
         * Perform local search operations in tree space for imputing  
         * missing data values in variable "Tree" using the MPE (Most  
         * Probable Explanation) criterion.
         */
        SearchTrees(DAG,Phy);
    ...
    result["DAG"] = WriteLandscape(SimDAG, Emtree, GeneLabels, pthres);
    return result;
}
...
void BootstrapAnalysis(vector< vector<bool> >& Data, vector<SimVar>& SimDAG, int& NRep, string& job)
{
    ...
    for(int Brep=1; Brep<NRep+1;Brep++)
    {
        ...
        for(size_t i=0;i<netrel2.size();i++)
        {
            for(size_t j=0;j<netrel2.size();j++)
            {
